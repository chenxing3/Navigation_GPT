...
# --- Configuration ---
RESULTS_FEATHER_FILE = '../dataset/trajectory/full_model/homing_trajectory/trajectories_iterative_with_echo_to_home_20251217_142455.feather' 
HEX_CENTERS_CSV = '../ops/token_info.csv'
ADDITIONAL_TRAJECTORIES_FILE = ''
ADDITIONAL_TRAJECTORIES_FILE2 = ''

ROW_INDEX_TO_PLOT = 24
...
def draw_transparent_plot(
    ...
    if azimuth_path_x_proj:
        ax.plot(azimuth_path_x_proj, azimuth_path_y_proj, color='#ffa15a', linewidth=5, solid_capstyle='round')

    # 4. Plot Selected Grid Points
    if grid_path_x_proj:
        colors = np.arange(len(grid_path_x_proj))
        ax.scatter(grid_path_x_proj, grid_path_y_proj, c=colors, cmap='rainbow', s=24, zorder=3)
...
# 5. Plot Sensory Locations - Enlarged, black fill, white border
    if middle_loc_plot_x:
        ...
        ax.scatter(middle_loc_plot_x, middle_loc_plot_y, marker='o', s=180, c='black',
                   edgecolors='white', linewidths=3, zorder=5)
        


    # 6. Plot Start (LOC1) and Target (LOC2)
    ax.plot(loc1_x, loc1_y, marker='*', markersize=24, color='red', linestyle='None')
    ax.plot(loc2_x, loc2_y, marker='X', markersize=24, color='black', linestyle='None')
    ...
    #     Line2D([0], [0], color='dimgray', lw=2.5, linestyle='--', label='Trained trajectory (forward)'),
    #     Line2D([0], [0], color='blue', lw=3, label='Generated trajectory'),
    #     Line2D([0], [0], marker='D', color='w', label='Sensory location',
    ...
    #     Line2D([0], [0], marker='o', color='w', label='Gird location',
